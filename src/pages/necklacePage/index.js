...
import styles from "./necklacePage.module.scss";
...
function NecklacePage() {
  ...
  const [categoryName, setCategoryName] = useState("Trang sức dây chuyền");
    ...
    const fetchProducts = async () => {
      ...
      try {
        ...
        const filteredProducts = data.filter(
          (product) => product.categoryName === "Trang sức dây chuyền"
        );

        if (filteredProducts.length > 0) {
          setCategoryName(filteredProducts[0].categoryName);
        }
